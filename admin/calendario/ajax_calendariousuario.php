...
switch ($_GET["accion"])
{
	case "listar_evento":
	{
		...
		if ($fila=$query->fetch_array())
		{
			do
			{
				echo "<p>".$fila["evento"]."<a href='#' class='eliminar_evento' rel='".$fila["id"]."' />";
			}
			...
			for ($i=1;$i<=$diasdespues;$i++)
			{
				if ($tr<$totalfilas)
				{
					if ($i>=$primeromes && $i<=$tope) 
					{
						...
						if (date("Y-m-d")<=$fecha_completa && es_finde($fecha_completa)==false) echo "<a href='#' data-evento='#nuevo_evento' title='Agregar un Evento el ".fecha($fecha_completa)."'  rel='".$fecha_completa."'>&nbsp;</a>";
